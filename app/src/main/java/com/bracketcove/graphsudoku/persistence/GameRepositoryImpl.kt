...
class GameRepositoryImpl(
    ...
    override suspend fun updateNode(
        ...
        when (val updateNodeResult: GameStorageResult = gameStorage.updateNode(x, y, color, elapsedTime)) {
            is GameStorageResult.OnSuccess -> onSuccess(
                puzzleIsComplete(updateNodeResult.currentGame)
            )
            is GameStorageResult.OnError -> onError(updateNodeResult.exception)
        }
    }
    ...
    override suspend fun getCurrentGame(
        ...
        when (val getCurrentGameResult: GameStorageResult = gameStorage.getCurrentGame()) {
            is GameStorageResult.OnSuccess -> onSuccess(
                getCurrentGameResult.currentGame,
                puzzleIsComplete(getCurrentGameResult.currentGame)
            )
            is GameStorageResult.OnError -> {
                // Case where the user has first loaded  the application, we need to create a new game
                // with settings
                when (val getSettingsResult = settingsStorage.getSettings()) {
                    is SettingsStorageResult.OnSuccess -> {
                        when (val updateGameResult: GameStorageResult =
